export const ProjectsData = [
  {
    id: 1,
    title: "DeFix3 - Monedero blockchain descentralizado",
    funcion: "Backend",
    image: require("../../assets/projects/defix3.png"),
    about:
      "Lorem Ipsum es simplemente el texto de relleno de las imprentas y archivos de texto. Lorem Ipsum ha sido el texto de relleno estándar de las industrias desde el año 1500, cuando un impresor (N. del T. persona que se dedica a la imprenta) desconocido usó una galería de textos y los mezcló de tal manera que logró hacer un libro de textos especimen. ",
    techs: [
      {
        icon: require("../../assets/techs/node-js.png"),
        name: "NodeJs",
      },
      {
        icon: require("../../assets/techs/typescript.png"),
        name: "TypeScript",
      },
      {
        icon: require("../../assets/techs/python.png"),
        name: "Python",
      },
      {
        icon: require("../../assets/techs/django.png"),
        name: "Django",
      },
    ],
    tags: [
      {
        icon: "fi fi-rr-globe",
        name: "Website",
        link: "https://www.defix3.com/#/",
      },
      {
        icon: "fi fi-brands-github",
        name: "Github",
        link: "https://github.com/iOchando/defix3_back",
      },
    ],
  },
  {
    id: 2,
    title: "MusicFeast - Donde los fanáticos de la música se unen",
    funcion: "Backend · Frontend Integration",
    image: require("../../assets/projects/musicfeast.png"),
    about:
      "Lorem Ipsum es simplemente el texto de relleno de las imprentas y archivos de texto. Lorem Ipsum ha sido el texto de relleno estándar de las industrias desde el año 1500, cuando un impresor (N. del T. persona que se dedica a la imprenta) desconocido usó una galería de textos y los mezcló de tal manera que logró hacer un libro de textos especimen. ",
    techs: [
      {
        icon: require("../../assets/techs/javascript.png"),
        name: "JavaScript",
      },
      {
        icon: require("../../assets/techs/vuejs.png"),
        name: "VueJs",
      },
      {
        icon: require("../../assets/techs/node-js.png"),
        name: "NodeJs",
      },
      {
        icon: require("../../assets/techs/typescript.png"),
        name: "TypeScript",
      },
      {
        icon: require("../../assets/techs/python.png"),
        name: "Python",
      },
      {
        icon: require("../../assets/techs/django.png"),
        name: "Django",
      },
    ],
    tags: [
      {
        icon: "fi fi-rr-globe",
        name: "Website",
        link: "https://www.musicfeast.io/",
      },
      {
        icon: "fi fi-brands-github",
        name: "Github",
        link: "https://github.com/iOchando/musicfeast_back_django",
      },
    ],
  },
  {
    id: 3,
    title: "Sistema de Planificación, Control y Gestión",
    funcion: "Backend · Frontend",
    image: require("../../assets/projects/planificacion.png"),
    about:
      "Lorem Ipsum es simplemente el texto de relleno de las imprentas y archivos de texto. Lorem Ipsum ha sido el texto de relleno estándar de las industrias desde el año 1500, cuando un impresor (N. del T. persona que se dedica a la imprenta) desconocido usó una galería de textos y los mezcló de tal manera que logró hacer un libro de textos especimen. ",
    techs: [
      {
        icon: require("../../assets/techs/javascript.png"),
        name: "JavaScript",
      },
      {
        icon: require("../../assets/techs/vuejs.png"),
        name: "VueJs",
      },
      {
        icon: require("../../assets/techs/python.png"),
        name: "Python",
      },
      {
        icon: require("../../assets/techs/django.png"),
        name: "Django",
      },
    ],
    tags: [
      {
        icon: "fi fi-brands-github",
        name: "Github",
        link: "https://github.com/iOchando/frontend_miplanificacion",
      },
    ],
  },
  {
    id: 4,
    title: "Mi Portafolio",
    funcion: "Frontend",
    image: require("../../assets/projects/portafolio.png"),
    about:
      "Lorem Ipsum es simplemente el texto de relleno de las imprentas y archivos de texto. Lorem Ipsum ha sido el texto de relleno estándar de las industrias desde el año 1500, cuando un impresor (N. del T. persona que se dedica a la imprenta) desconocido usó una galería de textos y los mezcló de tal manera que logró hacer un libro de textos especimen. ",
    techs: [
      {
        icon: require("../../assets/techs/react.png"),
        name: "React",
      },
      {
        icon: require("../../assets/techs/javascript.png"),
        name: "JavaScript",
      },
    ],
    tags: [
      {
        icon: "fi fi-brands-github",
        name: "Github",
        link: "https://github.com/iOchando/frontend_miplanificacion",
      },
    ],
  },
];
